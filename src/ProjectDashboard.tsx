    ...
    <div className="min-h-screen bg-[#0f1115] text-gray-200 flex font-sans selection:bg-architectural-yellow selection:text-black overflow-hidden relative">
      ...
      <aside className="w-64 border-r border-white/10 bg-black/40 backdrop-blur-2xl fixed h-full z-[60] flex flex-col p-6 shadow-2xl">
        <div className="flex items-center gap-3 mb-12">
          <div className="w-10 h-10 bg-architectural-yellow text-black flex items-center justify-center font-bold text-xl rounded-xl shadow-lg">R</div>
          <div className="font-mono text-[10px] font-bold uppercase tracking-widest leading-tight text-white">Arki Challenge</div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                ...
                {filtered.map(p => (
                  <motion.div whileHover={{ y: -8 }} key={p.id} onClick={() => setSelectedProject(p)} className={cn("bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden cursor-pointer hover:border-white/20 hover:shadow-2xl transition-all duration-500", submittedIds.includes(p.id) && "opacity-40 grayscale")}>
                    ...
                    <div className="p-6 space-y-5"><div className="flex flex-wrap gap-1.5">{p.software.slice(0, 3).map(s => <SoftwareBadge key={s} s={s} />)}</div><div className="flex items-center justify-between text-[10px] font-mono text-gray-500 uppercase border-t border-white/5 pt-4"><span>{p.category}</span><div className="flex items-center gap-1 text-white font-bold group-hover:text-architectural-yellow transition-colors uppercase">VIEW INQUIRY <Icons.ChevronRight size={12} /></div></div></div>
                  </motion.div>
                ))}
              </div>
            </section>
